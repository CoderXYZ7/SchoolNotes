...
class App:
    ...
    def get_value(string, char):
        value = 0
        pow_of_ten = 1
        for i in range(find_char(string, ';') + 1, len(string)):
            if string[i] != '.':
                value += (int(string[i]) - 48) * math.pow(10, pow_of_ten)
                pow_of_ten -= 1
        return value
    
    def get_id(string):
        res = (int(string[0]) - 48) * 10 + int(string[1]) - 48
        return res
    ...
    def get_string(string, char):
        ID = find_char(string, char)
        result = ""
        for i in range(ID):
            result += string[i]
        return result
    ...
    def load_map_int(file, Map):
        for line in file:
            s = line.strip()
            id = (int(s[0]) - 48) * 10 + int(s[1]) - 48
            s = s[3:]
            values = []
            while len(s) > 2:
                sign = -1 if s[0] == '-' else 1
                s = s[1:] if s[0] == '-' else s
                values.append(sign * ((int(s[0]) - 48) * 10 + int(s[1]) - 48))
                s = s[3:]
            Map[id] = values
    ...
    def load_turn(file):
        ...
        turn = {}
        turn['index'] = get_id(s[:2])
        turn['FOSchosen'] = get_string(s[3:], ';')
        turn['correct'] = is_correct(s)
        return turn
    
    def stamp_map(file, Map):
        for key, value in Map.items():
            file.write(f"{key};{value}\n")
    
    def new_turn(path):
        with open(path + "//output.txt") as cTurn:
            Turn = load_turn(cTurn)
    
        with open(path + "//pl//list.txt") as figOfSpeech:
            List = {}
            load_map(figOfSpeech, List)
    
        with open(path + "//gm//stats.txt") as statistics:
            People = {}
            load_map_int(statistics, People)
    
        People[Turn['index']].append(Turn['correct'] * int(List[Turn['FOSchosen']]))
        List[Turn['FOSchosen']] = sigmoid(inverted_sigmoid(abs(List[Turn['FOSchosen']])) - Turn['correct'])
    
        with open(path + "//pl//list.txt", "w") as figOfSpeechO:
            stamp_map(figOfSpeechO, List)
    
        with open(path + "//gm//stats.txt", "w") as PeopleO:
            for key, value in People.items():
                if key < 10:
                    PeopleO.write(f"0{key}|")
                else:
                    PeopleO.write(f"{key}|")
                for v in value:
                    PeopleO.write(f"{v};")
